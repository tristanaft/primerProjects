...
bool operator<(const StrVec& lhs, const StrVec& rhs){
    ...
    //ok, if here, we have hit the end of one of the vectors
    //return true if lhs ends and rhs does not end, otherwise false
    //they must be equal, since we went through all elements and they were =
    return lit == lhs.end() && rit != rhs.end();
}
bool operator>(const StrVec& lhs, const StrVec& rhs){return rhs < lhs;}
bool operator<=(const StrVec& lhs, const StrVec& rhs){return lhs < rhs || lhs == rhs;}
bool operator>=(const StrVec& lhs, const StrVec& rhs){return lhs > rhs || lhs == rhs;}
